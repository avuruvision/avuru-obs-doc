...
1. **eBPF (zero-code).** The `sensor` runs upstream **OBI** to capture zero-code
   traces, and the service **map** is derived from those spans — no propagation
   headers required.
...
not a separate agent. See [Feature status](/docs/status).
:::
