...
<ReleaseEntry version="0.7.0" date="in development" status="in-progress">

The `main` trunk. Planned direction (see the [Roadmap](/docs/roadmap)): more
clients — a CLI with `--fail-on` for CI gates and a documented Grafana recipe
over the hub API — services that declare their own domain, environment and
tier, richer auto-tagging from Kubernetes labels, and the inter-zone traffic
accounting whose design landed in v0.6.
