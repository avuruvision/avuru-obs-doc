...
<ReleaseEntry version="0.8.0" date="in development" status="in-progress">

The `main` trunk. Planned direction (see the [Roadmap](/docs/roadmap)): the
service map grows up — namespace and group boundaries, edge volume carried on
the edge itself, distinct treatment for external and undetected peers, and
**virtual targets** (databases, caches and message brokers as first-class nodes)
derived from the exit spans already stored. Plus a navigation structure that
