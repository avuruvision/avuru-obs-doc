...
<ReleaseEntry version="0.9.0" date="in development" status="in-progress">

The `main` trunk. Planned direction (see the [Roadmap](/docs/roadmap)): the mesh
and the kernel — recovering the real `app → app` dependency across a proxy hop,
the L4/L7 detail the kernel already sees, and mesh-facing surfaces for the
clusters where the mesh *is* the network.
