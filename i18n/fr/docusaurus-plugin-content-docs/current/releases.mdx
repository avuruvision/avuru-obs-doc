...
<ReleaseEntry version="0.9.0" date="en développement" status="in-progress" statusLabel="En cours">

Le tronc `main`. Direction prévue (voir la
[Feuille de route](/docs/roadmap)) : le maillage et le noyau — reconstituer la
vraie dépendance `app → app` à travers un saut de proxy, le détail L4/L7 que le
noyau voit déjà, et des surfaces dédiées au maillage pour les clusters où le
maillage *est* le réseau.
