...
|---|---|---|---|
| **`sensor`** | eBPF DaemonSet on every node — reconstructs the service map and captures flows, traces, RED, logs and profiles | exports **OTLP** | [eBPF mode](/docs/setup/ebpf-mode) |
...
1. A request hits one of your services. The node-local `sensor` sees the
   `tcp_connect()` / `tcp_close()` and the spans, with **no code change**.
